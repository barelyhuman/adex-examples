    ...
    <Layout>
      <form class="w-full flex flex-col gap-2" onSubmit={onFormSubmit}>
        <div class="flex justify-end">
          <button class="bg-neutral-800 text-white border border-transparent hover:border-white inline-flex items-center justify-center px-6 py-2 rounded-md">
            Create
          </button>
        </div>
        <div>{language}</div>
        <div>
          <input
            class="bg-neutral-800 w-full px-3 py-2 rounded-md"
            name="title"
            placeholder="Title"
          />
        </div>
        <textarea
          ...
          ref={node => {
            ...
            node.addEventListener('keyup', () => {
              ...
              const langFromCode = flourite(node.value, {
                shiki: true,
              }).language
